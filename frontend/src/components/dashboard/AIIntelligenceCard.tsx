...
export const AIIntelligenceCard: React.FC<AIIntelligenceCardProps> = ({
  ...
  return (
    <Card className="border-cyan-500/30 bg-gradient-to-b from-surface-200/90 to-surface-100/90 shadow-xl">
      ...
      <div className="flex items-center gap-2 mt-4 pb-2 border-b border-surface-border/60 overflow-x-auto">
        <button
      ...
      {activeTab === "risk" && (
        <div className="mt-4 space-y-4">
          {/* Top Score Banner */}
          <div className="p-4 rounded-xl bg-surface-100/80 border border-surface-border flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
            ...
            <div className="p-3 rounded-lg bg-surface-200 border border-surface-border text-xs shrink-0">
              <span className="text-[10px] font-mono text-slate-400 uppercase">
                Estimated Minimum Assets:
              </span>
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-1.5 font-mono">
                <span className="text-cyan-400">Boats: {initialRisk.resource_requirements.RESCUE_BOAT || 6}</span>
                <span className="text-rose-400">Ambulances: {initialRisk.resource_requirements.AMBULANCE || 4}</span>
                <span className="text-emerald-400">NDRF Teams: {initialRisk.resource_requirements.NDRF_TEAM || 4}</span>
                <span className="text-amber-400">Power Units: {initialRisk.resource_requirements.MOBILE_GENERATOR || 2}</span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {initialRisk.risk_factors.map((factor, idx) => (
                <div
                  key={idx}
                  className="p-3 rounded-lg bg-surface-100/50 border border-surface-border flex flex-col justify-between"
                >
                  <div>
                  ...
                  {factor.mitigation_hint && (
                    <div className="mt-2.5 pt-2 border-t border-surface-border/40 text-[10px] text-cyan-400 font-mono">
                      ↳ {factor.mitigation_hint}
                    </div>
                  )}
                </div>
            ...
            {initialResources.prioritized_resources.map((unit) => (
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="w-8 h-8 rounded-lg bg-surface-200 border border-surface-border flex items-center justify-center font-mono font-bold text-sm text-cyan-400 shrink-0">
                    #{unit.priority_rank}
                  </div>
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-sm font-semibold text-slate-100">{unit.unit_name}</span>
                      {getUrgencyBadge(unit.urgency)}
                      <span className="text-[10px] font-mono text-slate-500">[{unit.unit_code}]</span>
                    </div>
                    <p className="text-xs text-slate-400 mt-1 leading-relaxed">{unit.reason}</p>
                    <div className="mt-1.5 text-xs text-cyan-300 font-mono">
                ...
                <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center border-t sm:border-t-0 pt-2 sm:pt-0 border-surface-border/40 shrink-0 text-xs font-mono">
                  <div className="text-right">
                    ...
                    <span className="text-slate-100 font-semibold">{unit.distance_km} km</span>
                  </div>
                  <div className="text-right mt-0.5">
                    <span className="text-slate-400">ETA: </span>
                    <span className="text-emerald-400 font-semibold">{unit.estimated_transit_minutes} mins</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      {activeTab === "recommendations" && (
        <div className="mt-4 space-y-3">
          <div className="p-3 rounded-lg bg-surface-100/60 border border-surface-border text-xs text-slate-300">
            <span className="font-bold text-cyan-400 font-mono">Incident Strategy: </span>
          ...
          <div className="space-y-3">
            {initialRecommendations.recommendations.map((rec) => (
              <div
                key={rec.id}
                className="p-3.5 rounded-xl bg-surface-100/70 border border-surface-border space-y-2"
              >
