...
export const ResponsePlanCard: React.FC<ResponsePlanCardProps> = ({ initialPlan }) => {
  ...
  return (
    <Card className="border-emerald-500/30 bg-surface-100 shadow-xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-surface-border gap-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500/20 to-cyan-600/30 border border-emerald-500/40 flex items-center justify-center text-emerald-500 shrink-0">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 my-4">
        <div className="p-3 rounded-xl bg-surface-200 border border-surface-border">
          <span className="text-[10px] uppercase font-mono text-slate-500 dark:text-slate-400">Total Incidents</span>
        ...
        <div className="p-3 rounded-xl bg-surface-200 border border-surface-border">
          <span className="text-[10px] uppercase font-mono text-slate-500 dark:text-slate-400">Units Allocated</span>
          ...
          <span className="text-[10px] text-slate-400 font-mono">Ordered by Priority</span>
        </div>

        <div className="p-3 rounded-xl bg-surface-200 border border-surface-border">
          <span className="text-[10px] uppercase font-mono text-slate-500 dark:text-slate-400">Supply Shortages</span>
        ...
        <div className="p-3 rounded-xl bg-surface-200 border border-surface-border">
          <span className="text-[10px] uppercase font-mono text-slate-500 dark:text-slate-400">Avg Deployment ETA</span>
          ...
          <span className="text-[10px] text-slate-400 font-mono">Terrain-Modeled</span>
        </div>
      </div>

      {/* Interactive Tabs */}
      <div className="flex items-center gap-2 pb-2 border-b border-surface-border overflow-x-auto">
        <button
            ...
            {initialPlan.deployment_sequence.map((order) => (
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="w-8 h-8 rounded-lg bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center font-mono font-bold text-sm text-emerald-600 dark:text-emerald-400 shrink-0">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-600 dark:text-slate-300 mt-1">
                      ...
                      <span className="text-cyan-700 dark:text-cyan-300 font-medium">{order.incident_name}</span>
                    </p>
                ...
                <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center border-t sm:border-t-0 pt-2 sm:pt-0 border-surface-border shrink-0 text-xs font-mono">
                  <span className="text-slate-500 dark:text-slate-400">Estimated Transit:</span>
                  <span className="text-emerald-600 dark:text-emerald-400 font-bold text-sm mt-0.5">
                    ~{order.estimated_eta_minutes} mins
                  </span>
                  <span className="text-[10px] text-slate-400">(Terrain-Modeled)</span>
                </div>
              </div>
            ))}
          </div>
          ...
          {initialPlan.unresolved_shortages.length === 0 ? (
            ...
            initialPlan.unresolved_shortages.map((shortage, sIdx) => (
              ...
              >
                ...
                <div className="pt-2 border-t border-amber-500/20 text-[11px] text-amber-700 dark:text-amber-400 font-mono">
        ...
        <div className="mt-4 space-y-3">
          {initialPlan.operational_warnings.map((warn, wIdx) => (
            ...
            >
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-semibold text-rose-700 dark:text-rose-300">{warn.title}</span>
                  <span className="text-[10px] font-mono text-slate-500">[{warn.warning_code}]</span>
                </div>
                <p className="text-xs text-slate-700 dark:text-slate-300 mt-1 leading-relaxed">{warn.message}</p>
              </div>
            </div>
          ))}
        </div>
