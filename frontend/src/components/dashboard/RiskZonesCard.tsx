...
export const RiskZonesCard: React.FC<RiskZonesCardProps> = ({ riskSummary }) => {
  ...
  return (
    <Card>
      ...
      {/* Progress Bar Stack */}
      <div className="h-2.5 w-full rounded-full bg-slate-800 flex overflow-hidden gap-0.5 mb-4">
        {tiers.map((tier, idx) => (
          <div
            key={idx}
      ...
      <div className="grid grid-cols-2 gap-2.5">
        {tiers.map((tier, idx) => (
          <div
            key={idx}
            className="p-2.5 rounded-lg bg-surface-100/60 border border-surface-border flex flex-col justify-between"
          >
      ...
      <div className="mt-4 p-3 rounded-lg bg-rose-500/10 border border-rose-500/20 text-xs">
