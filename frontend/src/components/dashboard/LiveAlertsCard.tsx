...
export const LiveAlertsCard: React.FC<LiveAlertsCardProps> = ({ initialAlerts }) => {
  ...
  return (
    <Card className="border-rose-500/30 bg-surface-100 shadow-xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-surface-border gap-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-rose-500/20 to-red-600/30 border border-rose-500/40 flex items-center justify-center text-rose-500 shrink-0">
        ...
        {/* Filter Pills */}
        <div className="flex items-center gap-1 bg-surface-200 p-1 rounded-lg border border-surface-border shrink-0">
            ...
            <div
              ...
              className={`p-4 rounded-xl border transition-all ${
                alert.status === "ACTIVE"
                  ...
                  : "bg-surface-200 border-surface-border opacity-85"
              }`}
            >
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                <div className="space-y-1.5 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {alert.occurrence_count > 1 && (
                      <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-surface-200 text-slate-700 dark:text-slate-300 border border-surface-border">
                  ...
                  {alert.recommended_action && (
                    <div className="p-2.5 rounded-lg bg-surface-200 border border-surface-border text-xs text-cyan-700 dark:text-cyan-300 font-mono flex items-start gap-2">
                      <ChevronRight className="w-3.5 h-3.5 text-cyan-500 dark:text-cyan-400 shrink-0 mt-0.5" />
                      <span>
                        <strong className="text-slate-500 dark:text-slate-400">Directive: </strong>
                        {alert.recommended_action}
                      </span>
                    </div>
                  )}
                ...
                {/* Action Controls */}
                <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-2 shrink-0 border-t sm:border-t-0 pt-2 sm:pt-0 border-surface-border">
      ...
      <div className="mt-4 pt-3 border-t border-surface-border flex items-center gap-2 text-[10px] text-slate-500 font-mono">
        <Radio className="w-3 h-3 text-rose-500 animate-pulse shrink-0" />
        <span>
